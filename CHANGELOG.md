...
### Added
- **Live view with sound and talk-back over WebRTC** (#80) — the camera now advertises a WebRTC stream served through Home Assistant's bundled go2rtc, so the standard camera cards and the companion apps play the live video together with the intercom's audio instead of the silent MJPEG. Opening the card while the intercom is idle wakes it, showing the last snapshot until the live picture arrives (a cold panel can take up to half a minute). A card that captures the microphone — for example the [WebRTC card](https://github.com/AlexxIT/WebRTC) with `media: video,audio,microphone` — sends the voice to the panel: the first microphone packet answers the call, exactly like the attend button in the app, and the session then lasts the conversation time the server allows (90 s by default) instead of the preview limit. The stock Home Assistant card plays video and audio only until core adds microphone support. Browsers and the companion apps only grant the microphone over HTTPS. The MJPEG stream, snapshots, recordings and the `send_audio` service are unchanged.
...
### Changed
- **Live media keeps real-time pace on small boards** — the media sockets now use a 4 MB receive buffer and are read in bulk on every event-loop wake-up, and the JPEG encoding behind the MJPEG stream runs off the event loop. On a Raspberry Pi 4 with a WebRTC viewer connected, the intercom's audio used to arrive at about three quarters of real time and packets were dropped by the kernel under load; it now keeps pace with no drops.
- **WebRTC viewers get the intercom's own H264** — the video the panel sends is forwarded to go2rtc as it arrives instead of being decoded and encoded again. Delivery starts at the first keyframe, and the re-encoding path remains as a fallback.
- **Recordings keep the intercom's own video** — the MP4 is now written from the H264 the panel sends, with the call audio encoded alongside, instead of re-encoding a JPEG copy of every frame with `ffmpeg` when the call ends. Recordings are sharper (no JPEG generation in between), their duration matches the real call (frame timestamps are kept instead of assuming 25 fps), the audio starts where it started in the call, and saving is instant instead of a burst of encoding on a small board. `ffmpeg` is no longer used; the JPEG path only serves as a fallback when the encoded video cannot be captured.
- **Only keyframes are decoded unless someone is watching the MJPEG** — the still image, the visitor snapshot and the persisted preview only need a picture every keyframe, so the decoder and the JPEG encoder now skip the frames in between. Every frame is decoded again, from the next keyframe on, while an MJPEG client (`/api/camera_proxy_stream`, a card with `camera_view: live` on a Home Assistant version without WebRTC) is connected. On a Raspberry Pi 4 a live session now costs about a third of a core instead of 1.2 (measured with the panel's keyframe every ~2 s); a WebRTC viewer with microphone adds about 0.7 of a core, an MJPEG viewer about 0.4.
...
### Fixed
- **Switching camera keeps the video** (#74) — the panel answers a source change by tearing the current room down and announcing a new one over push, but only `Autoon` and `Call` notifications started a stream, so the camera went idle a second after switching and the new view never appeared. `ChangeVideoSource` pushes now start a session for the room they carry, like auto-on does.
