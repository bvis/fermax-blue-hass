...
- **WebRTC viewers get the intercom's own H264** — the video the panel sends is forwarded to go2rtc as it arrives instead of being decoded and encoded again, so a viewer costs about a fifth of a core on a Raspberry Pi 4 on top of the ~1.2 cores a live session already takes for the MJPEG preview and the recording. Delivery starts at the first keyframe, and the re-encoding path remains as a fallback.
