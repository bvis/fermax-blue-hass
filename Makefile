...
.PHONY: lint format format-check typecheck deadcode test check cli pre-push dev-image extract-credentials check-recordings
...
# On demand, not in CI: the MP4s in DIR (e.g. copied from /media/fermax_recordings) must play in browsers
check-recordings: dev-image
	docker run --rm -v $(PWD):/app -v $(abspath $(DIR)):/recordings:ro -w /app -e FERMAX_RECORDINGS=/recordings $(DEV_IMG) pytest tests/test_streaming.py -q --no-cov -k real_recordings
