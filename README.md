...
- **The session ends on its own**, after the configured stream duration (or the 90 s conversation once you have talked), exactly like in the app. The card then keeps the last picture and switches to the next session on its own (a ring, the camera preview button), without waking the intercom; that session is watched only, so reopen the card with the microphone to talk.
